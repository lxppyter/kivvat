...
import { Server, RefreshCw, Loader2, Database, HardDrive, Shield, Laptop, Network, Cloud, Plus, Smartphone, AppWindow, Calendar } from "lucide-react";
...
export default function AssetsPage() {
  ...
  const handleCreate = async () => {
      ...
      try {
          ...
          setIsAddOpen(false);
          setNewAsset({ name: '', type: 'WORKSTATION', provider: 'ENDPOINT', serialNumber: '', assignedTo: '', bitlocker: false, antivirus: false, licenseKey: '', licenseExpiry: '' });
          fetchAssets();
      } catch (e) {
          console.error(e);
      ...
      <div className="flex items-center justify-between border-b border-border/60 pb-8">
        ...
        <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
            ...
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle>Envantere Kayıt Ekle</DialogTitle>
                </DialogHeader>
                
                <Tabs defaultValue="hardware" value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <TabsList className="grid w-full grid-cols-2">
                        <TabsTrigger value="hardware">Donanım & Cihaz</TabsTrigger>
                        <TabsTrigger value="software">Yazılım & Lisans</TabsTrigger>
                    </TabsList>
                ...
                <DialogFooter>
                    <Button onClick={handleCreate} disabled={creating}>
                        ...
                        {activeTab === 'hardware' ? 'Cihazı Kaydet' : 'Lisansı Kaydet'}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
