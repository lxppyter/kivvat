...
let ReportService = class ReportService {
    ...
    async generateFullPdfReport(reportId) {
        ...
        const getStandardName = (code) => {
            ...
            if (code.startsWith('Req'))
                return 'PCI DSS';
            if (code.startsWith('TEKNIK'))
                return 'KVKK / GDPR';
            return 'General Best Practice';
        };
        for (const res of results) {
            const ruleId = res.ruleId;
            const codes = scanner_service_1.RULE_TO_CONTROLS_MAP[ruleId] || [];
            const uniqueStandards = new Set(codes.map(getStandardName));
            if (uniqueStandards.size === 0)
                uniqueStandards.add('General Best Practice');
            for (const std of uniqueStandards) {
                if (!standardsStats[std])
                    standardsStats[std] = { total: 0, passed: 0, score: 0 };
                if (!groupedFindings[std])
                    groupedFindings[std] = [];
                standardsStats[std].total++;
                if (res.status === 'COMPLIANT')
                    standardsStats[std].passed++;
                groupedFindings[std].push({ ...res, mappedCodes: codes.filter((c) => getStandardName(c) === std) });
            }
        }
    ...
    async generateExportBundle(reportId) {
